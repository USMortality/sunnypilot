...
from openpilot.sunnypilot.selfdrive.controls.lib.speed_limit import PCM_LONG_REQUIRED_MAX_SET_SPEED, CONFIRM_SPEED_THRESHOLD, \
  UNLIMITED_SPEED_LIMIT_THRESHOLD_KPH
...
def speed_limit_pre_active_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  ...
  unlimited_speed_limit_threshold_conv = round(UNLIMITED_SPEED_LIMIT_THRESHOLD_KPH if metric else
                                               UNLIMITED_SPEED_LIMIT_THRESHOLD_KPH * CV.KPH_TO_MPH)
  speed_limit_confirm_conv = round(V_CRUISE_MAX if metric else V_CRUISE_MAX * CV.KPH_TO_MPH) if \
    speed_limit_final_last_conv >= unlimited_speed_limit_threshold_conv else speed_limit_final_last_conv
  alert_1_str = ""
  ...
  if CP.openpilotLongitudinalControl and CP.pcmCruise:
    ...
    if IS_MICI:
      if set_speed_conv < speed_limit_confirm_conv:
        alert_1_str = "Press + to confirm speed limit"
      elif set_speed_conv > speed_limit_confirm_conv:
        alert_1_str = "Press - to confirm speed limit"
    else:
