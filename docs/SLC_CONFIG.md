...
Once qualified, the existing lookahead and adaptation distances select the limit immediately, without an additional target delay. Brief unqualified entries ahead do not change the target. Current car and map limits apply immediately in both directions; the previous three-second increase delay has been removed. Late upcoming entries also bypass qualification when the normal braking distance is reached or the boundary is within 1.5 seconds of travel. This preserves reaction to late restrictions, but those entries can still flicker.
