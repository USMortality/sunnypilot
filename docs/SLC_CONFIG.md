...
The resolver applies initial limits and reductions immediately. An increase must remain the selected limit for three continuous seconds before it changes the target. For example, `70 → 50 → 70 → 50` within a few seconds stays at `50` after the first reduction. Returning to the lower value, changing the proposed increase, or losing all valid limits resets confirmation. This applies to all source policies, so genuine higher limits also have a three-second confirmation delay.

During confirmation, the retained lower limit has no lookahead distance: normal speed control handles it rather than treating an obsolete map position as permission to coast. Missing data is still reported as invalid; the existing last-known-limit behavior remains available. This filter suppresses brief increases but cannot correct a persistently incorrect map limit or prevent the first reaction to an erroneous lower limit.
